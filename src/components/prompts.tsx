...
この形式に沿って、現在の会話履歴を分析し、効果的な引き継ぎプロンプトを作成してください。`,
   ...
   mail_to_action: '以下のメール用文章を社外クライアント及び上司の関係性に対しての文章として適切な文章にリファクタリングしてください。目的は簡潔に、依頼及び要望、アクションは明瞭に、結論ファーストで出力を希望します。多くは非プログラマのため専門用語の出力は避け、極端な専門用語は一般用語に変換または注釈を追記してください。\n\n```\n\n```',
   mail_to_hairyo: '以下のメール用文章を社外クライアント及び上司の関係性に対しての文章として適切な文章にリファクタリングしてください。目的は簡潔に、依頼及び要望、アクションは明瞭に、相手を慮り配慮した出力を希望します。多くは非プログラマのため専門用語の出力は避け、極端な専門用語は一般用語に変換または注釈を追記してください。\n\n```\n\n```'
};
...
export const prompts_list: Value[] = [
   ...
   { label: "[user] クライアントメール用応答", value: prompts.mail_to_action },
   { label: "[user] クライアントメール用配慮", value: prompts.mail_to_hairyo }
];